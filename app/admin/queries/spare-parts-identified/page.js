...
import QuotesIdentify from './quotes-identify';
import QuotesAssigned from './quotes-assigned';

const URL_ORDERS   = 'repuestosporidentificar/por-identificar';
const URL_ASSIGN   = 'repuestosporidentificar/asignar-items';
const URL_UNASSIGN = 'repuestosporidentificar/quitar-items';

const TAB_KEYS = ['identify', 'assigned'];
...
export default function SparePartsIdentified() {
  ...
  const tabLabels = [t.quotes_with_codes_to_identify, t.assigned_quotations];
  ...
  return (
    <>
      ...
      <div className="animate__animated animate__faster animate__fadeIn">
        {activeTab === 0 && (
          <QuotesIdentify t={t} data={orders_unassigned} assignOrder={assignOrder} />
        )}
        {activeTab === 1 && (
          <QuotesAssigned t={t} data={orders_assigned} unassignOrder={unassignOrder} setOrdersAssigned={setOrdersAssigned} setOrdersUnassigned={setOrdersUnassigned} />
        )}
      </div>
    </>
  );
