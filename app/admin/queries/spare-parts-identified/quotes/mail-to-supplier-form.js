...
const url_data_email = process.env.NEXT_PUBLIC_API_URL + 'repporidentificar/DatosMailPrv';
const url_send_email = process.env.NEXT_PUBLIC_API_URL + 'repporidentificar/EnviarMailPrv';
const url_data_contacts = process.env.NEXT_PUBLIC_API_URL + 'repporcotizar/ContactosPrv';
...
const MailToSupplierForm = ({ close, t, item, token, order, order_id, print, selected = [] }) => {
        ...
        data.push(
          {
            ...
            DescripcionItem: item.DesRepuesto,
  ...
  const handleChangeSupplier = async (select) => {
    if (select == null) {
      setFilteredItems([]);
    }
    setValue('supplier', select);
    setSelectContacts([]);
    setValue('to', '');
    try {
      if (select.value != null) {
        const rs = await axios.post(url_data_contacts, { CodPrv: select.value, ValToken: token });
  ...
  return (
    <>
      <div className="flex sm:flex-row flex-col">
        <label className="text-end pt-2 mb-0 sm:w-1/4 sm:ltr:mr-2 rtl:ml-2" htmlFor="supplier">{t.supplier}</label>
