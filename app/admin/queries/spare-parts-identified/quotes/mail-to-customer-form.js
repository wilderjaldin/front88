...
const URL_DATA_EMAIL = (nroCotizacion) => `repuestosporidentificar/datos-mail-cliente/${nroCotizacion}`;
const URL_CONTACTS   = (codCliente)    => `repuestosporidentificar/contactos-cliente/${codCliente}`;
const URL_SEND_EMAIL = 'repuestosporidentificar/enviar-mail-cliente';
...
const MailToCustomerForm = ({ close, t, order, codCliente }) => {
  ...
  const getData = async () => {
    try {
      const rs = await axiosClient.get(URL_DATA_EMAIL(order.nroCotizacion));
  ...
  const refreshContacts = () => {
    ...
    axiosClient.get(URL_CONTACTS(codCliente))
          ...
          <tbody>
            {contacts.map((c, i) => (
              <tr key={contactKey(c) || i}
                ...
                <td className="px-2 py-1.5 text-slate-600 text-[12px]">{c.email1}</td>
