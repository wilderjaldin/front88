...
const URL_DATA_EMAIL = 'repuestosporcotizar/datos-mail-cliente';
const URL_CONTACTS   = (codCliente) => `repuestosporcotizar/contactos-cliente/${codCliente}`;
const URL_SEND_EMAIL = 'repuestosporcotizar/enviar-mail-cliente';
...
const MailToCustomerForm = ({ close, t, selected = [] }) => {
  const authUser   = useSelector(selectUser);
  const userEmail  = authUser?.Email ?? authUser?.email ?? '';

  const codCliente = selected[0]?.codCliente;
