...
const CarrierCard = ({ c, onDelete, t }) => (
  <div className="rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden">
    ...
    <div className="px-4 py-3 space-y-1.5 text-xs">
      ...
      <div className="flex flex-wrap gap-1 mt-2">
        ...
        {c.EsRepresentacion && <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary">{t.representation}</span>}
      </div>
    </div>
...
export default function CarrierList() {
  ...
  const handleDelete = (carrier) => {
    Swal.fire({
      title: t.delete_carrier_question,
      ...
      try {
        ...
        if (rs.data.estado === 'Ok') {
          await loadCarriers();
          Swal.fire({ title: t.deleted, icon: 'success', confirmButtonColor: '#15803d', confirmButtonText: t.close });
        }
      } catch (e) {}
    });
  };
          ...
          <div className="flex flex-col gap-1.5 min-w-[280px]">
            <div className="relative">
              <input
                ...
                placeholder={t.search_carrier_ph}
            ...
            {activo ? (
              <div className="flex flex-wrap items-center gap-1.5">
                <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full ${activo === 'AC' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {t.active}: {activo === 'AC' ? t.actives : t.inactives}
              ...
              <p className="text-[11px] text-gray-400">
                {t.prefixes} <span className="font-mono">activo:AC</span> · <span className="font-mono">activo:IN</span>
              </p>
            )}
          </div>
        ...
        <>
          {paginated.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
              {paginated.map((c, i) => (
                <CarrierCard key={i} c={c} onDelete={handleDelete} t={t} />
              ))}
            </div>
          )}
          {totalPages > 1 && (
