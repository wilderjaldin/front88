...
const URL_LIST      = '/admin/register/company/representatives';
...
export default function RepresentanteFormPage() {
  ...
  useDynamicTitle(isEdit ? 'Editar Representante' : 'Registrar Representante');
  ...
  const {
    ...
    defaultValues: {
      ...
      tipMoneda:          null,
      porFee:             '',
      blnIvaEnPrecio:     false,
      blnEsRepresentante: false,
      nomDestinoEntrega:  '',
    },
  });
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const repres = detailRes?.data ?? null;
        const listaP = ctrl.paises  ?? [];
        const listaM = ctrl.monedas ?? [];
        const listaT = ctrl.tipos   ?? [];
        ...
        if (repres) {
          ...
          const monedaObj = listaM.find(m => m.value === repres.tipMoneda) ?? null;
          const tipoObj   = listaT.find(tp => tp.value === repres.tipRep)  ?? null;
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-5 gap-3">
              <div className="col-span-2">
                <F label="Tipo" error={errors.tipRep}>
                  <Controller name="tipRep" control={control}
                    render={({ field }) => (
                      <Select {...field} options={tipos} isClearable
                        placeholder={t.select_option}
                        instanceId="select-tipo-rep" classNamePrefix="react-select"
                      />
                    )}
                  />
                </F>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-6 pt-1">
              <Checkbox id="blnIvaEnPrecio" label="Incluir IVA en precio" register={register} />
              <Checkbox id="blnEsRepresentante" label="Es Representante" register={register} />
            </div>
