...
export default function RepresentativeOwnLayout({ children }) {
  ...
  const TABS = [
    { key: 'general',    label: tr.general_information  },
    { key: 'parameters', label: tr.transport_parameters },
    { key: 'address',    label: tr.delivery_addresses   },
  ];
  ...
  if (!isAdmin && !isRepresentante) {
    return <AccessDenied message={tr.no_permission_section} />;
  }
