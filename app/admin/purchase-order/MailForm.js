...
const url_data_email = process.env.NEXT_PUBLIC_API_URL + 'ordcompradetalle/DatosProveedorMail';
const url_send_email = process.env.NEXT_PUBLIC_API_URL + 'ordcompradetalle/EnviarMailOrdenCompraPrv';
...
const MailForm = ({ close, t, item, token, order, order_id, print }) => {
  ...
  const getData = async () => {
    try {
      const rs = await axios.post(url_data_email, { NomProveedor: order.NomPrv, ValToken: token });
    ...
    try {
      ...
      if (rs.data.estado == 'Ok') {
        Swal.fire({
          ...
          text: t.the_purchase_order_was_generated,
          confirmButtonText: t.close
        }).then(() => {
          close();
          return;
    ...
    <>
      ...
      <form className="space-y-4" onSubmit={handleSubmit(onSend)}>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          <button onClick={() => noSend()} type="button" className="btn btn-outline-dark">
            { t.do_not_send } <IconArrowForward></IconArrowForward>
          </button>

          <button type="button" disabled={sending} onClick={handleSubmit(onSend)} className="btn btn-success disabled:opacity-50">
            {(sending) ? t.sending_message : t.btn_send}  <IconArrowForward></IconArrowForward>
          </button>
