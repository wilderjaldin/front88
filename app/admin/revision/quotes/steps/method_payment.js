...
const MethodPaymentQuote = ({
  ...
  const PaymentCard = ({ id, title, subtitle, selected, onSelect }) => (
    ...
    >
      <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide">{title}</span>
      <span className="text-lg font-bold text-gray-800 dark:text-gray-100 uppercase">{subtitle}</span>
      {selected && (
  ...
  const InfoLine = ({ label, value, bold }) => (
    <div className="flex items-center justify-between py-2.5 border-b border-dashed border-gray-200 dark:border-gray-700 last:border-0">
      <span className="text-sm text-gray-600 dark:text-gray-300">{label}</span>
      <span className={`text-sm ltr:ml-auto rtl:mr-auto ${bold ? 'font-semibold text-primary' : 'text-gray-800 dark:text-gray-100'}`}>{value}</span>
    </div>
  );

  return (
    <div className="flex justify-center">
      <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">

        <h2 className="text-base font-semibold text-gray-700 dark:text-gray-100 mb-5">{t.select_payment_method}</h2>
        ...
        {option_payment === 'TB' && (
          <div className="space-y-4">
            <div>
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">{t.make_the_deposit}</h3>
            ...
            <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">{t.send_the_receipt}</h3>
            ...
            <div className="flex items-start gap-3 rounded-xl bg-secondary/5 dark:bg-secondary/10 border border-secondary/20 p-4">
              <IconInfoCircle className="text-secondary shrink-0 mt-0.5 h-5 w-5" />
              <p className="text-sm text-gray-600 dark:text-gray-300">{t.order_will_be_processed}</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={f.name} className="flex items-center gap-3">
                <label className={`w-28 shrink-0 text-sm font-medium text-gray-500 dark:text-gray-400 text-right${f.required ? ' required' : ''}`}>{f.label}</label>
