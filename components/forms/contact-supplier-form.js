...
const url = process.env.NEXT_PUBLIC_API_URL + 'proveedor/AdicionarContactoPrv';

const ComponentContactSupplierForm = ({ action_cancel, contact = [], is_new = false, supplier = {}, token, updateListContact}) => {
  ...
  useEffect(() => {
    if (contact) {
      reset({ name: contact.NomContacto, last_name: contact.ApllContacto, position: contact.NomCargo, email_1: contact.Mail1, email_2: contact.Mail2, phone_1: contact.NumTelf1, phone_2: contact.NumTelf2, phone_3: contact.NumTelf3 });
    } else {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      let data_contact = {
        ...
        Mail1: data.email_1,
        Mail2: data.email_2,
        NumTelf1: data.phone_1,
        NumTelf2: data.phone_2,
        NumTelf3: data.phone_3,
        ValToken: token
      ...
      if (rs.data.estado == 'OK') {
        ...
        Swal.fire({
          ...
          updateListContact(rs.data.dato);
          action_cancel();
        });
      } else {
        Swal.fire({
...
export default ComponentContactSupplierForm;
